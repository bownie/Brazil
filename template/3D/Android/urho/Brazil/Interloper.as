...
class Interloper : GameObject
{
    ...
    float aimY;
	Node@ interloperNode;
	Vector3 m_position = Vector3(-10, 0, 0);

    Interloper(Vector3 position)
    {
        health = maxHealth = 2;
        okToJump = false;
        smoke = false;
		...
		//SetMessage("Creating Interloper");
	...
	void buildBody()
	{
		...
		interloperNode.position = m_position;
		interloperNode.SetScale(1);

		StaticModel@ object = interloperNode.CreateComponent("StaticModel");
        object.model = cache.GetResource("Model", "Models/Coin.mdl");
        ...
        headObject.material = cache.GetResource("Material", "Materials/BrightBlueUnlit.xml");
